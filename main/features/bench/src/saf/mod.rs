...
impl BenchRunner {
    ...
    pub fn new(config: BenchConfig) -> Self {
        let knee_detector: Arc<dyn KneeDetector> =
            match config.knee_detection.algorithm.as_str() {
    ...
    pub async fn run(
        ...
        Ok(LoadReport { steps, knee_index, policy })
    }
}
